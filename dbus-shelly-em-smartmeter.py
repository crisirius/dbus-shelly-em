...
class DbusShellyemService:
  def __init__(self, servicename, paths, productname='Shelly EM', connection='Shelly EM HTTP JSON service'):
    ...
    self.channel = int(config['DEFAULT'].get('Channel', 0))  # Kanal 0 als default
    ...
    self._dbusservice.add_path('/ProductId', 45069) # Beispielwert ET340 Energy Meter
    self._dbusservice.add_path('/DeviceType', 345)  # Beispielwert ET340 Energy Meter
    ...
    gobject.timeout_add(250, self._update) # pause 250ms before next request
